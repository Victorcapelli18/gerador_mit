...
class MitPresenter:
    ...
    def definir_schema(self, schema: dict) -> None:
        """Define o schema para validação JSON."""
        self.schema = schema
    ...
    def gerar_json(self) -> None:
        """Inicia o processo de geração de JSON."""
        ...
        try:
            ...
            # Função de callback para atualização do progresso
            def atualizar_progresso(valor: float) -> None:
                self.view.atualizar_progresso(valor)

            # Criação do repositório e caso de uso
